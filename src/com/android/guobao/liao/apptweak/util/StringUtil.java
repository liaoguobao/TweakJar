...
public class StringUtil {
    ...
    static public byte[] bufferToByte(ByteBuffer buf, boolean flip) {
        if (!buf.hasRemaining()) {
            return new byte[0];
        }
        if (buf.hasArray()) {
            int ofs = buf.arrayOffset();
            byte[] ba = Arrays.copyOfRange(buf.array(), ofs + (flip ? buf.position() : 0), ofs + (flip ? buf.limit() : buf.position()));
            return ba;
        } else {
